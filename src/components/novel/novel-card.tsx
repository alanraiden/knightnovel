...
export function NovelCard({ novel, rank }: { novel: DemoNovel; rank?: number }) {
  return (
    <Link
      ...
      className="group block rounded-card transition-all duration-200 ease-out hover:-translate-y-1.5"
    >
      <div className="relative aspect-[2/3] overflow-hidden rounded-card border border-transparent bg-card shadow-md shadow-black/0 transition-all duration-200 ease-out group-hover:border-accent-highlight/60 group-hover:shadow-xl group-hover:shadow-black/40">
        {novel.cover ? (
          <Image
            src={novel.cover}
            alt={novel.title}
      ...
      <p className="mt-2 truncate text-[13px] text-text-primary group-hover:text-accent-highlight">
        {novel.title}
      </p>
